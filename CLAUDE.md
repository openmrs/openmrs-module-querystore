...
- **Sync = events** ([Decision 12](docs/adr.md#decision-12-sync-mechanism--events-first-aop-as-last-resort-gap-filler)). `CoreServiceEventListener` consumes core's #6084 `*ServiceEvent`s (module targets 2.9) and projects via `RecordProjector` — the sole sync path. The AOP migration bridge and the `querystore.syncMode` switch were removed once events was verified end-to-end (no production deployments to soak). The surviving infra (`RecordProjector`, `RecordIndexer`, `AfterCommitDispatcher`, `SyncExecutor`) was renamed off its historical `bridge`-package name into the neutral `sync` package once the bridge was gone. CDC (#6151 + the pre-alpha debezium module) is the reserved heavier-coverage path.
