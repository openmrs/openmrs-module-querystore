...
Where event coverage is incomplete in the supported core version, the preferred remedy is to patch core to emit the missing event. AOP is permitted only as a targeted gap filler — scoped to specific entity types where patching core is not feasible — and is treated as tech debt to be removed once core catches up. The querystore-owned AOP bridge (below) is the concrete instance of this rule for deployments on core older than 2.9; on 2.9+ it is replaced wholesale by the core events and retired.
...
**Session-lifecycle constraint (load-bearing).** `OpenmrsServiceEventAdvice` publishes the event *inside* the originating transaction (before `proceed()`, with `@Order(50)` placing the aspect inside the transaction advice). This rules out a naive `@TransactionalEventListener(AFTER_COMMIT)`: it runs after the session closes and the entity is detached, so lazy navigation during serialization throws. A synchronous `@EventListener`, by contrast, runs at publish time with the session still open — but the cost to avoid is running the *whole* pipeline inline, because embedding (the expensive step) would block the clinical thread and the write would index state a later rollback discards. The split that resolves both is the one the bridge already uses: serialize synchronously while the session is open (cheap, and required for lazy access), then defer only embed + write to after commit (`AbstractIndexingAdvice` + `AfterCommitDispatcher`). The events-first handlers preserve it — a synchronous `@EventListener` **serializes at publish time** (in-transaction, session open), then registers a `TransactionSynchronization.afterCommit` callback (via `AfterCommitDispatcher`) to **embed + write after commit** — so embedding never blocks the clinical thread, nothing is indexed for a rolled-back transaction, and lazy navigations resolve against the still-open session. `AfterCommitDispatcher` is retained for this reason; it is not bridge-only scaffolding.
...
Core's event infrastructure ([#6084](https://github.com/openmrs/openmrs-core/pull/6084)) is merged on `2.9.x` but the module supports core 2.8+, and 2.9 is not yet released. During this gap — and for any deployment still on 2.8 — querystore needs a working write path so that consumers, chartsearchai in particular, can migrate to the read store on a realistic timeline rather than waiting on a core upgrade. On 2.9+ the bridge is selected off in favour of the core events (see [Runtime sync-mode selection](#runtime-sync-mode-selection-28--29-migration-control)). The bridge:
...
- **Per-type aspect classes, not a mega-aspect.** One class per resource type keeps the `args[0] instanceof <SpecificType>` filtering clean on services that host several types (next bullet). (Originally this split was also meant to allow type-by-type retirement; since core's single aspect emits for all types at once, retirement is now a single cutover instead — see the long-term-target bullet below.)
...
- **The events-first model remains the long-term target, and the target is now concrete.** The bridge retires *to core's `*ServiceEvent`s* ([#6084](https://github.com/openmrs/openmrs-core/pull/6084)), not to a querystore-owned event subscriber. Because core's single aspect emits for all types at once (rather than landing type-by-type as originally anticipated), the parity verification is done once across all bridge-covered types: the same save/void/retire/purge must produce the same read-store document via the core event as via the aspect. Double-firing during the verification overlap is harmless because `index()` is upsert-idempotent. Once parity holds on 2.9, every aspect and the `config.xml` `<advice>` block are deleted in one cutover.

The bridge is a deliberate, time-bound widening of AOP's scope, not a supersession. Aspects added under the bridge that outlive core's `*ServiceEvent` coverage are tech debt by definition and must be tracked accordingly.

### Runtime sync-mode selection (2.8 → 2.9 migration control)

The bridge and the core-events consumer cannot both fire for the same entity type without double work (correctness is safe — `index()` is idempotent — but each record would be serialized and embedded twice; embedding is the expensive operation). They also have different minimum core versions: the bridge runs on 2.8+, the core events require 2.9. A single global property selects the active path:

- **`querystore.syncMode`** ∈ `{aop, events, both}`, default `aop`.
  - `aop` — the querystore-owned migration bridge (the only option on core < 2.9).
  - `events` — consume core's `*ServiceEvent`s; the bridge aspects gate themselves off.
  - `both` — run both, for the parity-verification overlap only. **Not a steady state** (double embedding cost); the GP description must say so.
- **The toggle lives inside the pipeline, not in bean wiring.** Bridge advice is registered via `config.xml` `<advice>` at module load and cannot be unregistered at runtime, so the single gate is the early-return in `AbstractIndexingAdvice.afterReturning()` — one check disables all aspects. The core-events listeners are gated symmetrically.
- **`events` fails loud on a misconfigured stack.** Selecting `events` without core 2.9 / the `*ServiceEvent` classes on the classpath must fail at startup, not silently index nothing — a silently-stale projection is the worst failure mode for a CQRS read store. No silent fallback to `aop`.
- **Deprecation trajectory.** This GP is a migration control, not a permanent co-equal-options switch (which would contradict the "AOP is tech debt" stance above). Trajectory: default `aop` now → flip default to `events` once the module baselines on 2.9 and parity passes → delete the `aop` and `both` modes (and the bridge) together. `querystore.syncMode` itself is removed when only one path remains.
- **Granularity.** A single global mode is intentionally coarser than per-type cutover. Per-type overrides (e.g. `querystore.syncMode.obs=events`) are deferred unless an incremental cutover need actually surfaces — mirror the single-selector shape of [`querystore.backend`](#decision-3-pluggable-backend-spi-with-three-reference-implementations) rather than building a per-type matrix speculatively.

This subsection retires together with the bridge.
