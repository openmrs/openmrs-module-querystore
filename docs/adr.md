...
| `resource_uuid` | Link back to the source record in OpenMRS via the REST API (e.g., the obs UUID, condition UUID, order UUID, or allergy UUID depending on the resource_type) |
