...
- **(b) Mechanism.** Delete-loser + reindex-winner, *not* in-place `patient_uuid` rewrite. On the event the consumer (`CoreServiceEventListener.reconcileMerge`) defers to after-commit and runs `bulkDeleteByPatient(loser)` + `reindexPatient(winner)` (the [bootstrap](#initial-backfill--bootstrap) slice's delete-then-unconditional-reproject under a per-patient lock). An in-place rewrite was rejected because the per-record save events the merge *incidentally* fires through the proxy cover only the types core re-saves there (encounter, visit, non-encounter obs, patient program) — **not** orders/conditions/allergies/diagnoses/medication-dispense or encounter-contained obs, which have no merge helper and would linger under the loser uuid. Reprojecting the winner's chart from core (which now owns all the data) is authoritative and idempotent, independent of which reassignments emitted events. The in-flight window is bounded by the after-commit dispatch: the loser's `querystore_patient` row is removed by the `voidPatient` `VoidServiceEvent` (Decision 10), the loser's remaining docs by `bulkDeleteByPatient`, and the winner is rebuilt atomically under its per-patient lock. (`CoreServiceEventListenerTest` pins the routing and the malformed-log no-op.)
