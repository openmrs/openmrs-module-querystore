...
**SHORT-name promotion and cap.** SHORT-typed `ConceptName` entries (clinical abbreviations like `DTG` for Dolutegravir, `HTN` for Hypertension, `T2DM` for Type 2 Diabetes — how clinicians actually type queries) are returned unconditionally, ahead of and outside the cap. Non-SHORT synonyms remain capped at `MAX_NON_SHORT_SYNONYMS` (currently `10`, raised from the original `3` once the description-indexing slice established the BM25-only synonym channel — index-size pressure went down once synonyms stopped sharing embedding-input budget). The cap is on the non-SHORT bucket only; SHORT names are kept regardless of count. Same-string entries appearing in both buckets are deduplicated. Why split the buckets: under a single alphabetic cap, drug abbreviations were lost when locale-equivalent FQN variants happened to sort earlier — a search for `"DTG"` then missed the underlying dolutegravir record entirely. The promotion-and-raise split keeps the cap's IDF discipline for verbose dictionaries while guaranteeing the clinical abbreviations survive.
