...
The body is **earned, not mandatory.** Its only job is to carry what no single inline line can: the aggregate verdict (merge / needs-work), triage across several findings, a cross-cutting concern with no line to sit on, or a direct answer to a question the author asked on the PR. When the inline comments already carry everything actionable and there's no aggregate call to add beyond them, the body is a single verdict sentence — or, if even that would just be "LGTM," nothing at all (Step 4). A multi-paragraph cover note that re-tells each inline finding is the default failure mode; resist it.
...
> "Neither the summary verdict nor any inline comment's first line leads with a stamped label — not `issue:` / `suggestion:` / `nit:`, and not the em-dash/colon variants (`Disposition: merge`, `Question — …`, `Non-blocking suggestion: …`); each says in plain language what it is and what the author must do, and either requests an action or asks a question. Every blocking comment contains its failure-mode sentence; and no comment — fresh or threaded reply — exists merely to acknowledge a fix or credit resolved work."

> "The summary body carries only what no inline line can — the aggregate verdict and, at most, a bare pointer to what's left to do; when the inline comments already carry the findings it shrinks to a single verdict sentence (or nothing). It never restates a finding — a pointer is a bare reference, not a one-line recap of what the comment says (a body paragraph reproducing an inline comment is the finding said twice). Zero verified-clean or cleared-concern content anywhere in it: no 'what I checked' line, bullet, section, or aside, and none folded into prose — and on a re-review, no 'prior points addressed' recap, since a resolved finding earns silence, not a tally. The verification I did shows up only as the confidence behind the verdict, never written out; if a result must be on the record it is a single inline `note`, not body text."
